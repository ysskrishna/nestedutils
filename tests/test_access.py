...
def test_delete_list_index_disallowed():
    d = {"a": [1, 2, 3]}
    with pytest.raises(PathError):
        delete_at(d, "a.1")  # must explicitly allow list deletion
...
def test_get_list_index_non_integer():
    d = {"a": [1]}
    assert get_at(d, "a.x") is None  # default
...
def test_get_at_invalid_type_int():
...
def test_get_at_invalid_type_none():
    """get_at should raise PathError for None path"""
    d = {"a": 1}
    with pytest.raises(PathError) as exc_info:
        get_at(d, None)
    assert exc_info.value.code == PathErrorCode.INVALID_PATH
...
def test_get_at_invalid_type_dict():
    """get_at should raise PathError for dict path"""
    d = {"a": 1}
    with pytest.raises(PathError) as exc_info:
        get_at(d, {"key": "value"})
    assert exc_info.value.code == PathErrorCode.INVALID_PATH
...
def test_get_at_invalid_type_tuple():
    """get_at should raise PathError for tuple path"""
    d = {"a": 1}
    with pytest.raises(PathError) as exc_info:
        get_at(d, ("a", "b"))
    assert exc_info.value.code == PathErrorCode.INVALID_PATH
...
def test_set_at_invalid_type_int():
...
def test_set_at_invalid_type_none():
    """set_at should raise PathError for None path"""
    d = {}
    with pytest.raises(PathError) as exc_info:
        set_at(d, None, "value")
    assert exc_info.value.code == PathErrorCode.INVALID_PATH
...
def test_set_at_invalid_type_dict():
    """set_at should raise PathError for dict path"""
    d = {}
    with pytest.raises(PathError) as exc_info:
        set_at(d, {"key": "value"}, "value")
    assert exc_info.value.code == PathErrorCode.INVALID_PATH


def test_delete_at_invalid_type_int():
...
def test_delete_at_invalid_type_none():
    """delete_at should raise PathError for None path"""
    d = {"a": 1}
    with pytest.raises(PathError) as exc_info:
        delete_at(d, None)
    assert exc_info.value.code == PathErrorCode.INVALID_PATH
...
def test_delete_at_invalid_type_tuple():
    """delete_at should raise PathError for tuple path"""
    d = {"a": 1}
    with pytest.raises(PathError) as exc_info:
        delete_at(d, ("a", "b"))
    assert exc_info.value.code == PathErrorCode.INVALID_PATH
...
def test_get_after_set_symmetric():
    d = {}
    set_at(d, "a.0.b.0.c", "X")
    assert get_at(d, "a.0.b.0.c") == "X"
...
def test_set_at_creates_correct_types_auto():
    d = {}
